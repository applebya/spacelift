...
/*
  Scanning only the loaded page reports "no violations" about a fraction of
  the site: a closed disclosure, an unopened menu and an unselected tab panel
  are not in the accessibility tree, so nothing inside them can be flagged.
  Lighthouse shares that blind spot. It is how goodgradients scored
  accessibility 100 while carrying twenty nested-interactive violations one
...
for (const vp of VIEWPORTS) {
  ...
  for (const state of STATES) {
    if (state.only && state.only !== vp.name) continue
    try {
      /* Fresh load per state. Run sequentially on one page and an open menu
         sits over everything the next state needs to click, which reads as
         "could not reach" at best and silently scans the wrong thing at
         worst. */
      await prepare(page)
      await state.go(page)
    } catch (e) {
      console.log(
        `  ${state.name}: could not reach — ${String(e).slice(0, 70)}`
      )
      continue
    }
    ...
    if (violations.length === 0) {
      console.log(`  ${state.name}: no violations`)
    } else {
      for (const v of violations) {
        violationCount++
        console.log(`  ${state.name}: [${v.impact}] ${v.id} — ${v.help}`)
